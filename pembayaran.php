...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek hutang — single table, tidak ada ambiguity
    ...
    if (!$hutang) {
        ...
        $stmt = db()->prepare(
            "INSERT INTO pembayaran (hutang_id, user_id, jumlah_bayar, tanggal_bayar, metode, keterangan, bukti_gambar)
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        $stmt->bind_param('iidssss', $hid, $uid, $bayar, $tgl, $metode, $ket, $bukti);
        $stmt->execute();

        $sisaFloat = (float)$sisa_baru;
        db()->query("UPDATE hutang SET sisa=$sisaFloat, status='$status_baru' WHERE id=$hid");
        logActivity('PEMBAYARAN', "Hutang#$hid Bayar:$bayar Status:$status_baru");
        $msg = 'Pembayaran ' . formatRupiah($bayar) . ' berhasil dicatat.';
        if ($status_baru === 'lunas') $msg .= ' Hutang telah LUNAS! 🎉';
...
// ─── Filter hutang for form (prefix tabel eksplisit) ─
$whereH = $isAdm
    ? "h.status != 'lunas'"
    : "h.user_id = $uid AND h.status != 'lunas'";
$hutangList = db()->query(
    "SELECT h.id, h.kode, h.sisa, d.nama
     FROM hutang h
...
$total   = (int)db()->query(
    "SELECT COUNT(*) as c FROM pembayaran p
     JOIN hutang h ON p.hutang_id = h.id
     WHERE $whereP"
)->fetch_assoc()['c'];
...
$rows = db()->query(
    "SELECT p.*, h.kode, h.jenis, d.nama as debitur_nama, u.nama as petugas
     FROM pembayaran p
     JOIN hutang h   ON p.hutang_id  = h.id
     JOIN debitur d  ON h.debitur_id = d.id
...
// Pre-fill hutang_id dari GET (dipanggil dari halaman hutang)
...
<div class="card">
  <div class="card-header">
    <span class="card-title">
      <i class="fa-solid fa-money-bill-transfer" style="color:var(--success);margin-right:8px"></i>Riwayat Pembayaran
    </span>
    ...
    <table>
      ...
      <?php else: while ($row = $rows->fetch_assoc()): ?>
        <tr>
          ...
          <td>
            <a href="detail_hutang.php?id=<?= $row['hutang_id'] ?>"
               style="font-family:var(--mono);font-size:.78rem;font-weight:600;color:var(--accent)">
              <?= h($row['kode']) ?>
            </a>
          </td>
          ...
          <td style="font-family:var(--mono);font-weight:700;color:var(--success);font-size:.88rem">
          ...
          <?php if ($isAdm): ?>
            <td style="font-size:.8rem"><?= h($row['petugas']) ?></td>
          <?php endif; ?>
          <td>
            <?php if ($row['bukti_gambar']): ?>
              <img src="<?= $row['bukti_gambar'] ?>" class="img-preview"
                   onclick="openLightbox(this.src)" style="width:50px;height:40px">
            <?php else: ?>
  ...
  <div class="modal">
    <div class="modal-header">
      <span class="modal-title">
        <i class="fa-solid fa-money-bill-wave" style="color:var(--success);margin-right:8px"></i>Catat Pembayaran
      </span>
    ...
    <form method="POST" enctype="multipart/form-data">
      <div class="modal-body">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <div style="position:relative">
              ...
              <input type="number" name="jumlah_bayar" class="form-control" style="padding-left:36px" placeholder="0" min="1" step="any" required>
            </div>
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          <label>Bukti Pembayaran <span style="color:var(--text-muted);font-weight:400">(maks 2MB, JPG/PNG)</span></label>
